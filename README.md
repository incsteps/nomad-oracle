...
- **Auto-generated Configs**: Nextflow config and shell env files are generated by Terraform with the correct server IPs and credentials — no manual editing needed
...
- **MinIO**: S3-compatible object storage running on the Nomad server, accessible via public IP
...
# MinIO API credentials (service account for Nextflow)
# Generate with: openssl rand -base64 15 / openssl rand -base64 30
minio_api_access_key = "your-access-key"
minio_api_secret_key = "your-secret-key"

# FusionFS bucket name for Nextflow workDir
...
After successful deployment, Terraform will:
- Print connection details (Nomad URL, MinIO console, etc.)
- Auto-generate `generated/nextflow.config` with the correct server IPs and credentials
- Auto-generate `generated/env.sh` and `generated/env.fish` for shell setup

### 6. Running Nextflow Pipelines

Source the generated env file and run pipelines using the auto-generated config:
...
# fish
source generated/env.fish
```

Two profiles are available:

```bash
# FusionFS + MinIO (S3 workDir via MinIO on the server)
...
The generated config uses the server's actual public IP for both the Nomad address and MinIO S3 endpoint — no manual IP editing required.

> **Note**: The static config files in `nextflow/` are kept as `*.example` reference templates. Always use `generated/nextflow.config` for actual runs.
...
The easiest way is to source the generated env file:
...
This sets `NOMAD_ADDR` to the server's public IP. You still need the ACL token:

```bash
# Retrieve the bootstrap token from the server
ssh -i ./id_ed25519 ubuntu@<nomad_server_public_ip>
